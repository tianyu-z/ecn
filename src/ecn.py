...
from src.sampling import generate_test_batches, generate_training_batch, hash_batches
...
class State(object):
    def __init__(self, N, pool, utilities):
        ...
        self.N = N
        self.pool = pool
        self.utilities = torch.zeros(
            batch_size, 2, 3, dtype=torch.int64, device=FLAGS.device
        )
        self.utilities[:, 0] = utilities[0]
        self.utilities[:, 1] = utilities[1]

        self.last_proposal = torch.zeros(
            batch_size, 3, dtype=torch.int64, device=FLAGS.device
        )
        self.m_prev = torch.zeros(
            batch_size, FLAGS.utt_max_length, dtype=torch.int64, device=FLAGS.device
        )
...
def run_episode(
    batch, agent_models, batch_size, testing, render=False, initial_agent=0
):
    """
    ...
    num_steps = torch.full(
        (batch_size,), FLAGS.max_timesteps, dtype=torch.int64, device=FLAGS.device
    )
    ...
    current_A_proposal = torch.zeros(
        sieve.batch_size, 3, dtype=torch.int64, device=FLAGS.device
    )
    prev_A_proposal = torch.zeros(
        sieve.batch_size, 3, dtype=torch.int64, device=FLAGS.device
    )
    current_A_message = torch.zeros(
        sieve.batch_size, FLAGS.utt_max_length, dtype=torch.int64, device=FLAGS.device
    )
    prev_A_message = torch.zeros(
        sieve.batch_size, FLAGS.utt_max_length, dtype=torch.int64, device=FLAGS.device
    )
    ...
    for t in range(FLAGS.max_timesteps):
        if FLAGS.linguistic:
            ...
            _prev_message = torch.zeros(
                sieve.batch_size, 6, dtype=torch.int64, device=FLAGS.device
            )
        ...
        (
            ...
            this_proposal,
            _entropy_loss,
            _term_matches_argmax_count,
            _utt_matches_argmax_count,
            _utt_stochastic_draws,
            _prop_matches_argmax_count,
            _prop_stochastic_draws,
            _utt_mask,
            _prop_mask,
        ) = agent_model(
            pool=s.pool,
        ...
        if FLAGS.proposal_termination and not FLAGS.normal_form:
            term_a = torch.prod(this_proposal == _prev_proposal, dim=1, keepdim=True)
            ...
            if t % 2 == 1:
                ...
                term_a = torch.zeros(
                    (sieve.batch_size, 1), dtype=torch.uint8, device=FLAGS.device
                )

        elif FLAGS.proposal_termination and FLAGS.normal_form:
            if t % 2 == 1:
                term_a = torch.prod(
                    this_proposal == current_A_proposal, dim=1, keepdim=True
                )
            else:
                term_a = torch.zeros(
                    (sieve.batch_size, 1), dtype=torch.uint8, device=FLAGS.device
    ...
    return (
        ...
        num_policy_runs,
        utt_matches_argmax_count,
        utt_stochastic_draws,
...
def run(args):
    ...
    if args.wandb:
        ...
        wandb.init(
            project="ecn", name=args.name, dir=f"{args.savedir}", group=args.wandb_group
        )
    ...
    flags_dict = {
        flag.name: flag.value for flag in FLAGS.flags_by_module_dict()[main_list[0]]
    }
    ...
    test_batches = generate_test_batches(
        batch_size=args.batch_size, num_batches=5, random_state=test_r
    )
    ...
    if path.isfile(args.model_file) and not args.no_load:
        episode, start_time = load_model(
            model_file=args.model_file, agent_models=agent_models, agent_opts=agent_opts
        )
        print("loaded model")
    elif args.testing:
        print("")
        ...
        if render:
            ...
            for test_batch in test_batches:
                (
                    ...
                    entropy_loss_by_agent,
                    _term_matches_argmax_count,
                    _test_num_policy_runs,
                    _utt_matches_argmax_count,
                    _utt_stochastic_draws,
                    _prop_matches_argmax_count,
                    _prop_stochastic_draws,
                    _test_utt_mask_count,
                    _test_prop_mask_count,
                ) = run_episode(
                    batch=test_batch,
                    agent_models=agent_models,
            ...
            rewards_str = "%.2f,%.2f,%.2f" % (
                rewards_sum[0] / count_sum,
                rewards_sum[1] / count_sum,
                rewards_sum[2] / count_sum,
            )
            test_rewards_str = "%.2f,%.2f,%.2f" % (
                test_rewards_sum[0] / test_count_sum,
            ...
            print(
                "ep {}, {} games/sec, {:2.2f} avg steps".format(
                    episode,
            ...
            print(
                "argmaxp term={:4.4f} utt={:4.4f} prop={:4.4f}".format(
            ...
            if FLAGS.force_masking_comm:
                print(
                    "utt mask % {:2.2f},{:2.2f} test % {:2.2f},{:2.2f}".format(
                        *utt_mask_pct,
                        *test_utt_mask_pct,
                    )
                )
                print(
                    "prop mask % {:2.2f},{:2.2f} test % {:2.2f},{:2.2f}".format(
                        *prop_mask_pct,
                        *test_prop_mask_pct,
                    )
                )
    ...
    if not args.no_save and not args.testing:
        save_model(
            model_file=args.model_file,
            agent_models=agent_models,
            agent_opts=agent_opts,

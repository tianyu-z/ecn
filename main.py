...
flags.DEFINE_boolean(
    "linguistic", True, "whether agents can communicate along linguistic channel"
)
flags.DEFINE_boolean(
    "proposal", True, "whether agents can send propoposal along proposal channel"
)
...
flags.DEFINE_boolean(
    "utility_normalize", False, "sum of an agent utilities is ~max_utility"
)
flags.DEFINE_boolean(
    "utility_nonzero", False, "force min utility of 1 for every object"
)
flags.DEFINE_enum(
    "force_utility_comm",
    None,
    ["A", "B", "both"],
    "force an agent to communicate its utilities",
)
flags.DEFINE_float("prosociality", 0, "alpha of prosociality for selfish agents")
flags.DEFINE_boolean(
    "force_masking_comm",
    False,
    "force agents to communicate by masking their utilities",
)
flags.DEFINE_boolean(
    "random_start", False, "randomly choose which agent starts the negotiation"
)
flags.DEFINE_boolean(
    "proposal_termination",
    False,
    "term policy does nothing, to terminate agent must propose the same as the previous proposal",
)
...
def parse_flags(argv):
    ...
    parser.add_argument(
        "--test-seed", type=int, default=123, help="used for generating test game set"
    )
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument(
        "--episodes", type=int, default=5e5, help="total number of episodes to run"
    )
    # hyperparams
    ...
    parser.add_argument(
        "--testing", action="store_true", help="turn off learning; always pick argmax"
    )
    ...
    args.model_file = datetime.datetime.strftime(
        datetime.datetime.now(), args.model_file
    )

    if (FLAGS.force_masking_comm or FLAGS.force_utility_comm) and not FLAGS.linguistic:
